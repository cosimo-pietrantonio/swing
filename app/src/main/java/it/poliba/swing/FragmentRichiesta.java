    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        invia.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (UserItem.isEmpty()) {
                    if (realm.where(Offerta.class).equalTo("emailUtente", r.getMailUtente()).equalTo("luogoPartenza", et_LPartenza.getText().toString())
                            .equalTo("luogoArrivo", et_LArrivo.getText().toString()).equalTo("data", et_data.getText().toString()).count() == 0) {
                        if (realm.where(Offerta.class).equalTo("luogoPartenza", et_LPartenza.getText().toString())
                                .equalTo("luogoArrivo", et_LArrivo.getText().toString()).count() != 0) {
                            // inserire confronto su dataPartenza
                            final RealmQuery<Offerta> query = realm.where(Offerta.class).equalTo("luogoPartenza", et_LPartenza.getText().toString())
                                    .equalTo("luogoArrivo", et_LArrivo.getText().toString()).equalTo("data", et_data.getText().toString());
                            ...
                            while (resIter.hasNext()) {
                                ...
                                // da aggiungere discussione risultati (match riuscito e non)
                            }
                        }
                    } else {
                        Toast.makeText(getContext(), "C'è un offerta da te formulata con gli stessi parametri richiesti in questa data !! "
                                , Toast.LENGTH_LONG).show();
                    }
                }else {  final RealmQuery<Offerta_Periodica> queryP = realm.where(Offerta_Periodica.class)
                ...
                }else { Toast.makeText(getContext(),"Match semplice non riuscito",Toast.LENGTH_LONG).show(); }
